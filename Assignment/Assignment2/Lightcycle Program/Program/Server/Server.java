...
//                display("Server waiting for Clients on port " + port + ".");
    ...
    private synchronized boolean sendMessage(int id, String message) {
        // add timestamp to the message
        String time = sdf.format(new Date());
    ...
    class ClientThread extends Thread {
        ...
        public void run() {
            ...
            if(clients.size() == totalClients) {
                ...
                for(int i = 0; i < clients.size(); ++i) {
                    Lightcycle lightcycle = new Lightcycle(clients.get(i).username, clients.get(i).color, 1, (int) (Math.random() * 4), true);
                    lightcycle.addStep((int) (Math.random() * BOARD_SIZE), (int) (Math.random() * BOARD_SIZE), true);
